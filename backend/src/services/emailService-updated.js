...
const { getDefaultSMTPSettings } = require('../controllers/smtpController');
require('dotenv').config();

// Create reusable transporter object using database SMTP settings
const createTransporter = async () => {
  try {
    // First try to get SMTP settings from database
    const dbSMTPSettings = await getDefaultSMTPSettings();
    ...
    if (dbSMTPSettings) {
      ...
      return nodemailer.createTransporter({
        ...
        auth: {
          user: dbSMTPSettings.username,
          pass: dbSMTPSettings.password
        }
      });
    }
...
// Get FROM email and name from database settings or environment
const getFromEmailSettings = async () => {
  try {
    const dbSMTPSettings = await getDefaultSMTPSettings();
    if (dbSMTPSettings) {
      return {
        from: `"${dbSMTPSettings.from_name}" <${dbSMTPSettings.from_email}>`
      };
    }
  } catch (error) {
    console.error('Error getting FROM email settings:', error);
  }
...
async function sendEmail({ to, subject, text, html, attachments }) {
  try {
    ...
    const fromSettings = await getFromEmailSettings();

    const mailOptions = {
      from: fromSettings.from,
      to: to,
      subject: subject,
      text: text,
      html: html,
