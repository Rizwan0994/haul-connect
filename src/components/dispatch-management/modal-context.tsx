...
interface ModalContextType {
  ...
  openModal: (dispatchId: string, title: string) => void;
...
export const ModalProvider: React.FC<ModalProviderProps> = ({ children }) => {
  const [modals, setModals] = useState<DispatchModal[]>([]);
  ...
  const openModal = useCallback(
    (dispatchId: string, title: string) => {
      // Check if this dispatch modal is already open
      const existingModalIndex = modals.findIndex(
        (m) => m.dispatchId === dispatchId
      );
