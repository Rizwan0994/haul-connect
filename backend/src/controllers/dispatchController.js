...
const { dispatch: Dispatch, carrier: Carrier } = require("../models");
...
const createDispatch = async (req, res) => {
  try {
    ...
    res.status(201).json(successResponse("Dispatch created successfully", dispatch));
  } catch (error) {
    res.status(500).json(errorResponse("Error creating dispatch", error.message));
  }
};
...
const getAllDispatches = async (req, res) => {
  try {
    ...
    res.status(500).json(errorResponse("Error fetching dispatches", error.message));
  }
};
...
const getDispatchById = async (req, res) => {
  try {
    ...
    res.status(500).json(errorResponse("Error fetching dispatch", error.message));
  }
};
...
const updateDispatch = async (req, res) => {
  try {
    ...
    res.status(500).json(errorResponse("Error updating dispatch", error.message));
  }
};
...
const deleteDispatch = async (req, res) => {
  try {
    ...
    res.status(500).json(errorResponse("Error deleting dispatch", error.message));
  }
};
