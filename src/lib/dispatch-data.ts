...
export const getDispatchById = async (id: string): Promise<Dispatch | undefined> => {
  try {
...
export const createDispatch = async (dispatchData: Omit<Dispatch, "id">): Promise<Dispatch | undefined> => {
  try {
...
export const updateDispatch = async (
  ...
  try {
    const response = await backendApiClient.put(`/dispatches/${id}`, dispatchData);
    return response.data.data;
  } catch (error) {
